...
yapi/
  ...
  123/
    ...
    swagger.json           # generated by sm2tsservice
    json2service.json      # sm2tsservice config for this project only
  456/
    export.json
    swagger.json
    json2service.json
...
- Written automatically when syncing with a URL or passing `host` to `setup_json2service.js` / `sync_yapi_full.js`
...
- `sync_yapi_full.js` writes `export.json` (filtered) and regenerates swagger
...
After changing ignore rules, re-run `sync_yapi_full.js` for that project so `export.json` and `swagger.json` stay consistent.

## Prerequisites

`sm2tsservice` must be installed globally (3.2.0+):

```bash
npm i -g sm2tsservice
```

If missing, `ensure_sm2tsservice.js` will fail and ask the user to install it globally. Do not install it as a project devDependency.

`yapi/{project_id}/json2service.json` example:
...
- `remoteUrl` points to the local raw export file in the same directory
- Do not commit project tokens to git; read tokens from `YAPI_PROJECT_TOKEN_{project_id}`
- `sm2tsservice --quiet` runs with `cwd` set to `yapi/{project_id}/` so multiple projects do not overwrite each other
...
### 4. Setup Project Tooling

For each unique project ID, run in the current project directory:

1. `node scripts/ensure_sm2tsservice.js`
   - Verifies global `sm2tsservice` is available
   - If missing, instruct user to run `npm i -g sm2tsservice` and stop
2. `node scripts/setup_json2service.js <project_id> [host]`
   - Creates `yapi/config.json` if needed and registers the project ID
   - When `host` is provided, saves `yapi_host` to config
   - Creates or updates `yapi/{project_id}/json2service.json`
3. Optional: `node scripts/yapi_paths.js <project_id>` to inspect resolved paths and `yapi_host`

### 5. Sync Full YApi Data to Project
...
2. This will:
   ...
   - Save raw export to `yapi/{project_id}/export.json`
   - Run `sm2tsservice --quiet` inside `yapi/{project_id}/` when available
...
### 6. Extract Requested API Documentation
...
If extraction fails because the interface is missing locally, re-run step 5 to refresh the export file.

### 7. Present Results

1. Summarize synced local files per YApi project:
   - `yapi/{project_id}/export.json`
   - `yapi/{project_id}/swagger.json`
   - `yapi/{project_id}/json2service.json`
   - `yapi/config.json`
...
3. Run steps 3-5, then skip extraction
...
### ensure_sm2tsservice.js

Verifies `sm2tsservice` is installed globally. Exits with install instructions if not found.

### setup_json2service.js
...
- Runs `sm2tsservice --quiet` in `yapi/{project_id}/` when available
